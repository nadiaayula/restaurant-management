...
import { useDeleteTableMutation, useGetListTable } from '@/queries/useTable'
...
export const columns: ColumnDef<TableItem>[] = [
  ...
  },
  {
    accessorKey: 'token',
    header: 'QR Code',
    cell: ({ row }) => <div>{<QRCodeTable token={row.getValue('token')} tableNumber={row.getValue('number')} />}</div>,
  },
  {
    id: 'actions',
    enableHiding: false,
...
export default function TableTable() {
  ...
  const data = tableList.data?.payload.data ?? []
